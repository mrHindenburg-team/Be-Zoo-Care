...
struct ACKSessionResponse: Decodable {
    let url: String
}

enum ACKAPIError: LocalizedError {
    case invalidURL
...
final class ACKNetworkManager: Sendable {
    ...
    func fetchRegister(
        ...
        deviceID: String?,
        appsFlyerID: String
    ) async -> Result<ACKSessionResponse, ACKAPIError> {

        guard let url = URL(string: config.registerURL) else {
            ACKLogger.log(.error, "Register: invalid registerURL")
    ...
    func refresh(
        ...
        guard let url = URL(string: config.syncURL) else {
            ACKLogger.log(.error, "Sync: invalid syncURL")
            return
        }
        ...
        do {
            ...
            guard let http = response as? HTTPURLResponse else {
                ACKLogger.log(.error, "Refresh: invalid response")
                return
            }

            let rows = Self.bodyRows(body as [String: Any])
            ACKLogger.tableIfChanged("REFRESH", rows, id: "refresh")

            if !(200...299).contains(http.statusCode) {
                let responseBody = Self.responseBodyString(data)
                ACKLogger.log(.error, "Refresh: server error \(http.statusCode) — response: \(responseBody ?? "—")")
            }
        } catch {
            ACKLogger.log(.error, "Refresh: error — \(error.localizedDescription)")
        }
    }
    ...
    /// Converts a raw error-response body into a printable string for logs / ACKAPIError.
    ...
    private func performRequest<T: Decodable>(
        ...
        do {
            ...
            ACKLogger.table(tag, Self.bodyRows(body))

            guard (200...299).contains(http.statusCode) else {
                let responseBody = Self.responseBodyString(data)
                ACKLogger.log(.error, "\(tag): server error \(http.statusCode) — response: \(responseBody ?? "—")")
                return .failure(.serverError(http.statusCode, responseBody))
            }

            if http.statusCode == 204 || data.isEmpty {
                ACKLogger.log(.info, "\(tag): 204 / empty body → main")
            ...
            do {
                ...
                ACKLogger.log(.error, "\(tag): decoding error — \(error)")
