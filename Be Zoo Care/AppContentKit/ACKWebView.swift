...
struct ACKWebView: UIViewRepresentable {

    let urlString: String
    @ObservedObject var navState: ACKNavigationState
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        webView.customUserAgent = ACKConstants.userAgent
        ...
        if let url = URL(string: urlString) {
            ...
            ACKLogger.log(.debug, "WebView: load \(urlString)")
        } else {
            ACKLogger.log(.error, "WebView: invalid URL — \(urlString)")
        }
    ...
    private func makeConfiguration() -> WKWebViewConfiguration {
        ...
        config.userContentController.addUserScript(
            WKUserScript(
                source: ACKConstants.injectedScript,
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate {
        ...
        func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
            ...
            ACKLogger.log(.debug, "WebView: didStart — \(webView.url?.absoluteString ?? "")")
        }
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            ...
            updateNavButtons(webView)
            ACKLogger.log(.debug, "WebView: didFinish — \(webView.url?.absoluteString ?? "")")
        }
        ...
        func webViewWebContentProcessDidTerminate(_ webView: WKWebView) {
            ACKLogger.log(.warning, "WebView: process terminated — reload")
        ...
        private func handleError(_ error: Error, webView: WKWebView) {
            ...
            ACKLogger.log(.error, "WebView: error — \(error.localizedDescription)")
